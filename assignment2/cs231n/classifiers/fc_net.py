...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        layer_dim = np.hstack((input_dim, hidden_dims, num_classes))

        for i in range(self.num_layers):
            self.params['W%d' % (i + 1)] = weight_scale * np.random.randn(layer_dim[i], layer_dim[i + 1])
            self.params['b%d' % (i + 1)] = np.zeros(layer_dim[i + 1])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            fc_act, fc_cache[str(i + 1)] = affine_forward(X, self.params['W' + str(i + 1)],
